...
--inner join
select employee.ename,department.DeptName from employee inner join DEPARTMENT on EMPLOYEE.DEPTNO=DEPARTMENT.DEPTNO;


SELECT EMPLOYEE.ename,DEPARTMENT.DeptName FROM EMPLOYEE right JOIN DEPARTMENT ON EMPLOYEE.DEPTNO = DEPARTMENT.DEPTNO where EMPLOYEE.DEPTNO=60;

--left join
SELECT EMPLOYEE.ename,EMPLOYEE.DEPTNO
FROM EMPLOYEE LEFT JOIN DEPARTMENT
ON EMPLOYEE.DEPTNO = DEPARTMENT.DEPTNO AND DEPARTMENT.DeptName = 'IT';
...
SELECT empno, Ename
FROM (
    SELECT Empno,Ename,ROW_NUMBER() OVER (ORDER BY Empno) AS RoowNum
    FROM EMPLOYEE
     ) AS NumberedRows
WHERE RoowNum % 2 = 1;